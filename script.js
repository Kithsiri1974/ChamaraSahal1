...
function changeQty(itemName, sizeStr, priceVal, delta, itCode, maxStock = Infinity) {
    ...
    const overlayHtml = variations.map(v => {
      ...
      return `
        <div class="size-row ${hasQtyClass}" id="row-${cartKey}">
          <!-- Display item_size in yellow instead of it_code -->
          <span class="size-code">${sizeStr}</span>
          <div class="size-price-badge">
            <span class="price-tag">${priceVal.toFixed(2)}</span>
          </div>
          <!-- Stock displayed in normal size & yellow -->
          <span class="stock-badge">
            Stock: ${stockInHand}
          </span>
          <div class="qty-stepper">
            <button class="qty-btn" onclick="changeQty('${safeName}', '${sizeStr}', ${priceVal}, -1, '${safeCode}', ${stockInHand})">-</button>
